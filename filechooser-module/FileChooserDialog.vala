...
public class CustomFileChooserDialog : Object {
    ...
    public CustomFileChooserDialog (Gtk.FileChooserDialog dialog) {
        ...
        var files_preferences = new Settings ("io.elementary.plateau-gui.preferences");
        is_single_click = files_preferences.get_boolean ("single-click");

        var chooser_settings = new Settings ("io.elementary.plateau-gui.file-chooser");
    ...
    private void transform_w2_container (Gtk.Widget w2) {
        (w2 as Gtk.Container).get_children ().foreach ((w3) => {
            if (w3.get_name () == GTK_PATHBAR_PATH[3]) {
                ...
                if (tv != null) {
                    /* set its click behaviour the same as io.elementary.plateau-gui setting */
