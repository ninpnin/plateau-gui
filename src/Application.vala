...
public class Marlin.Application : Gtk.Application {
    ...
    static construct {
        ...
        app_settings = new Settings ("io.elementary.plateau-gui.preferences");
        icon_view_settings = new Settings ("io.elementary.plateau-gui.icon-view");
        list_view_settings = new Settings ("io.elementary.plateau-gui.list-view");
        column_view_settings = new Settings ("io.elementary.plateau-gui.column-view");
    }
    ...
    public override void startup () {
        ...
        if (Posix.getuid () == 0 && GLib.Environment.get_variable ("PKEXEC_UID") == null) {
            warning ("Running Files as root using sudo is not possible. " +
                     "Please use the command: io.elementary.plateau-gui-pkexec [folder]");
            quit ();
        };
    ...
    private int _command_line (ApplicationCommandLine cmd) {
        ...
        if (version) {
            cmd.print ("io.elementary.plateau-gui %s\n", Config.VERSION);
